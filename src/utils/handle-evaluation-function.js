/**
 * IBM Code Engine Function for Website Evaluation
 * 
 * This function evaluates website content against specified criteria using Watsonx.ai LLM.
 * 
 ...
 * }
 * 
 * Returns evaluation results with alignment scores (HIGH/MEDIUM/LOW) and reasoning.
 */

// Watsonx.ai configuration - these should be set as environment variables in Code Engine
...
/**
 * Call Watsonx.ai LLM to generate text
 */
async function generateTextWithLLM(prompt) {
  try {
    if (!WATSONX_API_KEY || !WATSONX_PROJECT_ID) {
      throw new Error('Missing Watsonx.ai configuration. Please set WATSONX_API_KEY and WATSONX_PROJECT_ID in your environment variables');
    }
 ...
 * This is the entry point that will be called by Code Engine
 */
async function main(args) {
  try {
    console.log('🚀 Website Evaluation Function Started');
    console.log('📝 Input received:', JSON.stringify(args, null, 2));

    // Extract input parameters
    const { websiteUrl, selectedSections, criteria } = args;
    ...
    const response = {
      ...
      evaluationSummary: {
        ...
        averageAlignment: results.reduce((acc, r) => {
          ...
          return acc + (scores[r.alignment] || 66);
        }, 0) / results.length,
        retryStats: {
    ...
    console.log('❌ Evaluation function error:', error.message);
...
// Export the main function for Code Engine
module.exports = { main };
